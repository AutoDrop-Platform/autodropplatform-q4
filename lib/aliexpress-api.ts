...
const mockProducts: Product[] = [
  {
    ...
    rating: 4.5,
    orders: 15420,
    seller: { name: "TechStore Official", rating: 4.8 },
    description:
  ...
  },
  {
    ...
    rating: 4.3,
    orders: 8750,
    seller: { name: "FitTech Store", rating: 4.6 },
    description:
  ...
  },
  {
    ...
    rating: 4.7,
    orders: 3250,
    seller: { name: "Modest Fashion House", rating: 4.9 },
    description:
  ...
  // (المزيد من المنتجات... كما هي بدون تغيير، تحافظ على البقية)
]
...
export async function getTrendingProducts(limit?: number): Promise<{ products: Product[]; total: number }> {
  ...
  return { products: trendingProducts, total: trendingProducts.length }
}
...
export async function getTopRatedProducts(limit?: number): Promise<{ products: Product[]; total: number }> {
  ...
  return { products: topRatedProducts, total: topRatedProducts.length }
}
...
export const colors = [
  "Black", "White", "Navy", "Gray", "Blue", "Pink", "Green", "Brown", "Beige", "Purple", "Teal", "Charcoal",
]
export const features = ["Travel-Friendly", "Quick-Dry", "Lightweight", "Wrinkle-Resistant", "GPS", "Waterproof"]

// ✅ Final export for main usage
